...
impl Operand {
    ...
    pub const fn size(&self) -> usize {
        OPERAND_TYPE_SIZE + match self {
            Operand::MemoryAddress(_) => MEMORY_ADDRESS_SIZE,
            Operand::Register(_) => REGISTER_SIZE,
            Operand::Flag(_) => FLAG_TYPE_SIZE,
            Operand::Immediate(_) => IMMEDIATE_VALUE_SIZE,
        }
    }
...
impl ResolvedUnit {
    ...
    pub const fn size(&self) -> usize {
        match self {
            ResolvedUnit::Instr(_) => INSTRUCTION_SIZE,
...
pub enum UnresolvedJump {
    ...
    JumpIfGreater(Operand, Operand, Label),
    JumpIfLess(Operand, Operand, Label),
    JumpIfEqual(Operand, Operand, Label)
}
...
impl UnresolvedJump {
    ...
    pub const fn size(&self) -> usize {
        match self {
            UnresolvedJump::Jump(_) => {
                INSTRUCTION_SIZE + MEMORY_ADDRESS_SIZE
            }
            UnresolvedJump::JumpIf(u, _) => {
                INSTRUCTION_SIZE + u.size() + MEMORY_ADDRESS_SIZE
            }
            UnresolvedJump::JumpIfNot(u, _) => {
                INSTRUCTION_SIZE + u.size() + MEMORY_ADDRESS_SIZE
            }
            UnresolvedJump::JumpIfGreater(u1, u2, _) => {
                INSTRUCTION_SIZE + u1.size() + u2.size() + MEMORY_ADDRESS_SIZE
            }
            UnresolvedJump::JumpIfLess(u1, u2, _) => {
                INSTRUCTION_SIZE + u1.size() + u2.size() + MEMORY_ADDRESS_SIZE
            }
            UnresolvedJump::JumpIfEqual(u1, u2, _) => {
                INSTRUCTION_SIZE + u1.size() + u2.size() + MEMORY_ADDRESS_SIZE
            }
        }
    }
    ...
    pub fn label(&self) -> &String {
        match self {
            ...
            UnresolvedJump::JumpIfGreater(_, _, s) => &s.0,
            UnresolvedJump::JumpIfLess(_, _, s) => &s.0,
            UnresolvedJump::JumpIfEqual(_, _, s) => &s.0,
        }
    }
    ...
    pub fn resolve(&self, real_addr: RamAddr) -> Vec<ResolvedUnit> {
        ...
        match self {
            ...
            UnresolvedJump::JumpIfGreater(o1, o2, _) => {
                res.push(ResolvedUnit::Instr(CPUInstr::JumpIfGreater));
                res.push(ResolvedUnit::Operand(*o1));
                res.push(ResolvedUnit::Operand(*o2));
                res.push(ResolvedUnit::Operand(Operand::MemoryAddress(real_addr)));
            },
            UnresolvedJump::JumpIfLess(o1, o2, _) => {
                res.push(ResolvedUnit::Instr(CPUInstr::JumpIfLess));
                res.push(ResolvedUnit::Operand(*o1));
                res.push(ResolvedUnit::Operand(*o2));
                res.push(ResolvedUnit::Operand(Operand::MemoryAddress(real_addr)));
            },
            UnresolvedJump::JumpIfEqual(o1, o2, _) => {
                res.push(ResolvedUnit::Instr(CPUInstr::JumpIfEqual));
                res.push(ResolvedUnit::Operand(*o1));
                res.push(ResolvedUnit::Operand(*o2));
                res.push(ResolvedUnit::Operand(Operand::MemoryAddress(real_addr)));
            },
...
impl Assembler {
    ...
    pub fn write_to_ram(&mut self, ram: &mut RAM) -> Result<(), ErrorType> {
        let mut start = ram.allocate(self.total_size())?;
        let bytes: Vec<u8> = self.as_bytes(start);

        for i in 0..bytes.len() {
            ram.write_byte(start, bytes[i])?;
            start.inc(1)?;
        }
    ...
    pub fn add_imm(&mut self, imm: ImmType) {
    ...
    /// Adds a [`CPUInstr::JumpIfGreater`] to the instructions with specified label
    pub fn add_jump_if_greater(&mut self, op1: Operand, op2: Operand, sym: Label) {
        self.add_unresolved_jump(UnresolvedJump::JumpIfGreater(op1, op2, sym));
    }

    /// Adds a [`CPUInstr::JumpIfLess`] to the instructions with specified label
